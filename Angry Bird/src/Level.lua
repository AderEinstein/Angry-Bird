...
function Level:init()
    ...
    function beginContact(a, b, coll)
        ...
        if types['Obstacle'] and types['Player'] then
            -- set flag on player upon collision with obstacle
            self.launchMarker.alien.hasCollided = true
...
function Level:update(dt)
    ...
    if self.launchMarker.launched then
        ...
        -- Check if all the trishotAliens are allmost done rolling and set flag
        local trishotStill = true
        if #self.trishotAliens > 0 then
            for k, trishotAlien in pairs(self.trishotAliens) do
                local dx, dy = trishotAlien.body:getLinearVelocity()
        ...
        -- if we fired our alien to the left or the alien/trishot is almost done rolling, respawn
        if xPos < 0 or ((math.abs(xVel) + math.abs(yVel) < 1.5) and trishotStill) then
        ...
        -- spawn trishot on press of spacebar if player alien hasn't collided yet AND user hasn't used trishot in current trial yet
        if not self.trishotTriggered and love.keyboard.wasPressed('space') and not self.launchMarker.alien.hasCollided then
            self.trishotTriggered = true
            for i = 1, 2 do
                local trishotAlien = Alien(self.world, 'round', xPos, yPos, 'Player')

                trishotAlien.fixture:setRestitution(0.4)
                trishotAlien.body:setAngularDamping(1)
                trishotAlien.body:setLinearVelocity(xVel, yVel + 30)
                trishotAlien.body:setLinearVelocity(xVel, yVel - 30)

                table.insert(self.trishotAliens, trishotAlien)
            end
        end
...
function Level:render()
    ...
    if self.trishotAliens then
        for k, alien in pairs(self.trishotAliens) do
            alien:render()
        end
    end
